...
largeShirts = runners.filter(runner => {
    return runner.shirt_size === "L" ? true : false;
})
...
ticketPriceTotal.push(runners.reduce((accumulator, currentRuner) => {
    return accumulator + currentRuner.donation;
}, 0));
...
// Problem 1
const biggestDonation = (runners.reduce((max, currentRuner) => {
    return Math.max(max,currentRuner.donation);
}, 0));

console.log("Biggest Donation:", biggestDonation);

// Problem 2

const biggestDonationRunner = (runners.reduce((max, currentRuner) => {
    if (Math.max(max.donation,currentRuner.donation) === max.donation) {
        return max;
    } else {
        return currentRuner;
    }
...
const countedSizes = (runners.reduce((allSizes, runner) => {
    if (runner.shirt_size in allSizes) {
        allSizes[runner.shirt_size]++;
    } else {
        allSizes[runner.shirt_size] = 1;
    }
    return allSizes ;
}, {}));
console.log("Counted Sizes:", countedSizes);
